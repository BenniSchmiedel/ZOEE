"""
In ``ZOEE.modules.functions`` all physical equations are defined which describe the energy balance of the earth system.
...
class flux_down:
    ...
    def insolation(self, funcparam):
        ...
        Input has to be given as `Dictionaries` supplied by ``ZOEE.modules.configuration.importer`` from a specific **configuration.ini**.

        **Function-call arguments** \n
    
        :param dict funcparams:     a dictionary of the functions parameters directly parsed from ``ZOEE.modules.models.model_equation``
                                        ...
                                        * *albedo*: The name of albedo function which is called from ``ZOEE.modules.functions.albedo`` to return the albedo value/distribution. See :doc:`class albedo <functions_code/albedo>`.
                                        ...
                                        * *solarinput*: Indicates whether the solar insolation distribution from ``climlab.solar.insolation`` are used (recommended for 1D EBMs), which are called from ``ZOEE.modules.functions.earthsystem().solarradiation``
                                        ...
                                        * *timeunit*: Determines which timeunit of the solarradiation shall be used for averaging (depending on how the *Base.stepsize_of_integration* is chosen*)

                                            * type:  string
                                            * unit: -
                                            * value: 'annualmean' (average annually and give :math:`Q` as Watt/m^2), 'year', 'month', 'day', 'second' 

                                        * *orbital*: Indicates whether the solar insolation considers manipulation through orbital parameters over time (this will replace ``ZOEE.modules.functions.earthsystem().solarradiation`` by ``ZOEE.modules.functions.earthsystem().solarradiation_orbital``
        ...
        if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:
            Vars.Read['solar'][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = Q_total
        ...
        if albedoread == True:
            ...
            if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:  # Only on 4th step (due to rk4)
                Vars.alpha = alpha
                Vars.Read['alpha'][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = alpha

        # Noise factor z on the solar insolation
        z = 0
        if Base.Runtime_Tracker % 4 == 0:
            if noise == True:
                # possible noisedelay which indicates a gap between updating the noise factor
                if (int(Vars.t / Base.stepsize_of_integration) % noisedelay) == 0:
                    # seed if same noise is desired
                    if seed == True:
                        np.random.seed(int(Vars.t) + seedmanipulation)
                    z = np.random.normal(0, noiseamp)
                    # write to Base and output
                Base.Noise_Tracker = z

        z = Base.Noise_Tracker
        if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:
            Vars.Read['noise'][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = z
        ...
        if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:  # Only on 4th step (due to rk4)
            Vars.Read['Rdown'][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = R_in
        return R_in
    ...
    def dynamic_bud(self, T_1, T_2, alpha_0, alpha_1, alpha_2):
        ...
        **Function-call arguments** \n
        ...
        if Base.parallelization == True:
            if np.shape(T_1) == ():
                T_1 = np.tile(T_1, Base.number_of_parallels)
            if np.shape(T_2) == ():
                T_2 = np.tile(T_2, Base.number_of_parallels)
            if np.shape(alpha_1) == ():
                alpha_1 = np.tile(alpha_1, Base.number_of_parallels)
            if np.shape(alpha_2) == ():
                alpha_2 = np.tile(alpha_2, Base.number_of_parallels)
            if np.shape(alpha_0) == ():
                alpha_0 = np.tile(alpha_0, Base.number_of_parallels)

            albedo = np.reshape(np.zeros(len(Vars.Lat) * Base.number_of_parallels),
                                (Base.number_of_parallels, len(Vars.Lat)))
            # if np.shape(T_1)==(Base.number_of_parallels,) and np.shape(T_2)==(Base.number_of_parallels,) and  \
            # np.shape(alpha_0)==(Base.number_of_parallels,) and np.shape(alpha_1)==(Base.number_of_parallels,) and np.shape(alpha_2)==(Base.number_of_parallels,):

            # print(T_1,T_2,alpha_0,alpha_1,alpha_2)
            for i in range(Base.number_of_parallels):
    ...
    def dynamic_sel(self, Z, b):
        ...
        **Function-call arguments** \n
        ...
        if Base.parallelization == True:
            albedo = np.reshape(np.zeros(len(Vars.Lat) * Base.number_of_parallels),
                                (Base.number_of_parallels, len(Vars.Lat)))
            if np.shape(Z) == (Base.number_of_parallels, len(Vars.Lat)):
                for i in range(Base.number_of_parallels):
                    for j in range(len(Vars.Lat)):
                        if Tg[i, j] < 283.16:
...
class flux_up:
    ...
    def budyko_noclouds(self, funcparam):
        ...
        **Function-call arguments** \n
        ...
        :param dict funcparams:     a dictionary of the function's parameters directly parsed from ``ZOEE.modules.ModelEquation.model_equation``
        ...
        if Base.parallelization == True:
            ...
            if type(Activation) == bool:
                ...
                R_out = np.reshape(np.zeros(Base.number_of_parallels * len(Vars.Lat)),
                                   (Base.number_of_parallels, len(Vars.Lat)))
                for i in range(Base.number_of_parallels):
            ...
            if Activation == False:
                ...
                R_out = -(A + B * (Vars.T - 273.15))
        if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:  # Only on 4th step (due to rk4)
            Vars.Read['Rup'][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = R_out
        return R_out
    ...
    def budyko(self, funcparam):
        ...
        **Function-call arguments** \n
        ...
        if Activated == True:  # with activation statement
            if Base.parallelization == True:
                dT = np.transpose(Vars.T_global - np.transpose(Vars.T))
                F = beta * dT
            else:
        ...
        **Function-call arguments** \n
        ...
        """if Base.parallelization==True:
            ...
            K_wv,K_h,K_o,g,a,eps,p,e0,L,Rd,dy,dp,cp,dz,l_cover,re,cp_w,dens_w,factor_wv,factor_air,factor_oc,factor_kwv,factor_kair=\
                [xr.DataArray(paras[i],coords=[np.arange(Base.number_of_parallels)],dims=['parallel']) if np.shape(paras[i])==(Base.number_of_parallels,) else xr.DataArray(paras[i],coords=[Vars.Lat],dims=['lat']) if np.shape(paras[i])==(len(Vars.Lat),) else xr.DataArray(paras[i],coords=[Vars.Lat2],dims=['lat2']) if np.shape(paras[i])==(len(Vars.Lat2),) else xr.DataArray(paras[i],coords=[np.arange(Base.number_of_parallels),Vars.Lat],dims=['parallel','lat']) if np.shape(paras[i])==(Base.number_of_parallels,len(Vars.Lat)) else paras[i] for i in range(len(paras))]
              """
        if Base.parallelization == True:
            paras = [K_wv, K_h, K_o, g, a, eps, p, e0, L, Rd, dy, dp, cp, dz, l_cover, re, cp_w, dens_w, factor_wv,
        ...
        if Activated == True:
            ...
            # Apply Base.parallelization if activated
            if Base.parallelization == True:
                P1, P0 = np.reshape(np.zeros(Base.number_of_parallels * len(Vars.Lat) * 2),
                                    (2, Base.number_of_parallels, len(Vars.Lat)))
                for i in range(Base.number_of_parallels):
                    P_0 = np.append(P[i], 0)
            ...
            if Readout == True:
                if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:
                    ...
                    for l in range(len(Readdata)):
                        Vars.Read[Readdatakeys[l]][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = Readdata[l]
        else:
            Transfer = 0
        return Transfer
...
class forcing:
    ...
    def predefined(self, funcparam):
        ...
        **Function-call arguments** \n
        ...
        if Base.Runtime_Tracker == 0:
            ...
            if timeunit == 'month':
                Vars.ExternalInput[forcingnumber][0] = lna(
                    Vars.ExternalInput[forcingnumber][0]) * 60 * 60 * 24 * 365 / 12
            if timeunit == 'year':
        ...
        while Vars.t > Vars.ExternalInput[forcingnumber][0][Vars.ForcingTracker[forcingnumber][0]]:
            if Vars.ForcingTracker[forcingnumber][0] == (len(Vars.ExternalInput[forcingnumber][0]) - 1):
                ...
                Vars.ForcingTracker[forcingnumber][1] = Vars.ExternalInput[forcingnumber][1][
                    Vars.ForcingTracker[forcingnumber][0]]
                Vars.ForcingTracker[forcingnumber][0] += 1
        F = Vars.ForcingTracker[forcingnumber][1] * k_output + m_output
        if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:
            Vars.ExternalOutput[forcingnumber][int(Base.Runtime_Tracker / (4 * Base.data_readout))] = F
        return F

    def predefined1d(self, funcparam):
        """ 
        The predefined1d forcing imports data containing external radiative forcings for a 1-dimensional grid.

        .. _Predefined1dforcing:
    ...
    def co2_myhre(self, funcparam):
        ...
        **Function-call arguments** \n
        ...
        F = Vars.CO2Tracker[1]
        if Base.Runtime_Tracker % (4 * Base.data_readout) == 0:
            Vars.CO2Output[int(Base.Runtime_Tracker / (4 * Base.data_readout))] = F
        return F
    ...
    def orbital(self, funcparam):
        ...
        This module imports the orbital parameters from a given datafile and updates them during a model run and determines the downward radiation. 

        **Function-call arguments** \n

        :param dict funcparams:     * *datapath*: The path to the file (give full path or relative path!)
                                    ...
                                    * *col_ecc*: The column where the eccentricity data is stored

                                        * type: int 
                                        * unit: -
                                        * value:  any

                                    * *col_per*: The column where the perihel data is stored

                                        * type: int 
                                        * unit: -
                                        * value:  any
                            
                                    * *col_obl*: The column where the obliquity data is stored
                                    ...
                                    * *initial*: Initial orbital parameters

                                        * type: dict 
                                        * unit: -
                                        * value: for present day {'ecc': 0.017236, 'long_peri': 281.37, 'obliquity': 23.446}
                              
                                    * *perishift*: Phaseshift, required if data is given as short perihel

                                        * type: float 
                                        * unit: degree
                                        * value: any (standard 180)
        ...
        while Vars.t > Vars.ExternalOrbitals[0][Vars.OrbitalTracker[0]]:
            if Vars.OrbitalTracker[0] == (len(Vars.ExternalOrbitals[0]) - 1):
                Vars.OrbitalTracker[1] = {'ecc': Vars.ExternalOrbitals[1][-1],
                                          'long_peri': Vars.ExternalOrbitals[2][-1] + perishift,
                                          'obliquity': Vars.ExternalOrbitals[3][-1]}
                break
            else:

                Vars.OrbitalTracker[1] = {'ecc': Vars.ExternalOrbitals[1][Vars.OrbitalTracker[0]],
                                          'long_peri': Vars.ExternalOrbitals[2][Vars.OrbitalTracker[0]] + perishift,
                                          'obliquity': Vars.ExternalOrbitals[3][Vars.OrbitalTracker[0]]}
    ...
    def solar(self, funcparam):
        ...
        **Function-call arguments** \n
        ...
        :param dict funcparams:     * *datapath*: The path to the file (give full path or relative path!)
                                    ...
                                    * *k_output*: Scaling factor on output data

                                        * type: float 
                                        * unit: -
                                        * value: any

                                    * *m_output*: Offset factor on output data

                                        * type: float 
                                        * unit: -
                                        * value: any

                                    * *k_input*: Scaling factor on input data

                                        * type: float 
                                        * unit: -
                                        * value: any

                                    * *m_input*: Scaling factor on input data (useful if only the anomaly is desired)
...
class earthsystem:
    ...
    def meridionalwind_sel(self, a, re):
        ...
        **Function-call arguments** \n
        ...
        if Base.parallelization == True:
            # v=xr.DataArray(np.array([[0]*len(Vars.Lat2)]*Base.number_of_parallels,dtype=float),coords=[np.arange(Base.number_of_parallels),Vars.Lat2],dims=['parallel','lat2'])
            v = np.reshape(np.zeros(Base.number_of_parallels * len(Vars.Lat2)),
                           (Base.number_of_parallels, len(Vars.Lat2)))
        ...
        if Base.parallelization == True:
            if np.shape(a) == (len(Vars.Lat2), Base.number_of_parallels):
                a = np.transpose(a)
            for l in range(Base.number_of_parallels):
                if np.shape(a) == (Base.number_of_parallels, len(Vars.Lat2)):
    ...
    def temperature_difference_latitudes(self):
        ...
        **Function-call arguments** \n
        ...
        if Base.latitudinal_circle == True:
            f = interpolator(Vars.Lat, Vars.T)
            if Base.both_hemispheres == True:
                Lat_new = np.linspace(-90, 90, int(180 / Base.spatial_resolution + 1))
            else:
                Lat_new = np.linspace(0, 90, int(90 / Base.spatial_resolution + 1))
    ...
    def area_latitudes(self, re):
        ...
        **Function-call arguments** \n
        ...
        A = 2 * np.pi * re ** 2 * (sind(lat_northbound) - sind(lat_southbound))
...
def plotmeanstd(array):
    ...
    arraymean = np.mean(array[:][-int(Base.eq_condition_length):], axis=0)
    arraystd = np.std(array[:][-int(Base.eq_condition_length):], axis=0)

    # for l in range(len(arraynew)):
    #    arraymean.append(np.mean(arraynew[l][-Base.eq_condition_length:]))
    #    arraystd.append(np.std(arraynew[l][-Base.eq_condition_length:]))
...
def SteadyStateConditionGlobal(Global):
    ...
    if dT <= Base.eq_condition_amplitude:
        return True
...
def BPtimeplot(time, number):
    time_new = (lna(time) / Base.stepsize_of_integration - Vars.External_time_start[number])
    return time_new
