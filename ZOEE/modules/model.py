"""
``ZOEE.modules.models`` determines the structure of the EBM from functions given to it. The operation of this modules is adding the given functions :math:`F_1,F_2,...F_i` according to the following scheme (compare :doc:`physical background <../models>`):
...
with the deviation function :math:`y=\\frac{dT}{dt}` required by the ``ZOEE.modules.rk4.rk4alg`` and :math:`C_{ao}` the heat capacity of the system which is passed to the right side of the model equation.
...
def model_equation(eqparam, funccomp):
    ...
    **Function-call arguments** \n
    ...
    :param dict funccomp:       Configuration 2D dictionary containing function names and function parameters used:
                                    ...
                                    * funcnames: a dictionary of names of functions defined in ``ZOEE.modules.functions`` which are added up. See :doc:`here <functions>` for a list of functions
