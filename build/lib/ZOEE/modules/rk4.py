"""
The ``ZOEE.modules.RK4`` provides the numerical scheme to iteratively solve differential equations, hence the :doc:`model equation <modelequation>` which is parsed by ``ZOEE.modules.models``, initialized with the :doc:`configuration <configuration>` provided by ``ZOEE.modules.configuration``.
...
from .model import model_equation
...
def rk4alg(config, progressbar=True, monthly=False):
    from tqdm import tqdm, tnrange
    eqparam = config['eqparam']
    rk4input = config['rk4input']
    funccomp = config['funccomp']
    """This function is the core module performing the numerical approximation of the evolution. 

    explained above by using the solution of the model equation from ``ZOEE.modules.models``. 
    ...
    The required input-argument is the configuration dictionary returned by ``ZOEE.modules.configuration.importer``.


    **Function-call arguments** \n
    

    :param dict config:         The model configuration file. It contains all information below
    ...
    n, h = int(Base.number_of_integration), Base.stepsize_of_integration
    ...
    if Base.spatial_resolution > 0:
        if Base.parallelization:
            data = [np.zeros(n + 1), np.zeros((n + 1, Base.number_of_parallels, len(Vars.Lat))),
                    np.zeros((n + 1, Base.number_of_parallels))]
        else:
            data = [np.zeros(n + 1), np.zeros((n + 1, len(Vars.Lat))), np.zeros(n + 1)]
    else:
        if Base.parallelization:
            data = [np.zeros(n + 1), np.zeros((n + 1, Base.number_of_parallels)),
                    np.zeros((n + 1, Base.number_of_parallels))]
        else:
    ...
    for i in progress:
        ...
        T0 = Vars.T
        k1 = h * model_equation(eqparam, funccomp)
        Base.Runtime_Tracker += 1
        Vars.T = T0 + 0.5 * k1
        k2 = h * model_equation(eqparam, funccomp)
        Base.Runtime_Tracker += 1
        Vars.T = T0 + 0.5 * k2
        k3 = h * model_equation(eqparam, funccomp)
        Base.Runtime_Tracker += 1
        Vars.T = T0 + k3
        k4 = h * model_equation(eqparam, funccomp)
        Base.Runtime_Tracker += 1
        ...
        if monthly:
            ...
            day = int(i % (365 / 12))
            if day == 15:
                Base.Readout_Tracker += 1

                data[0][Base.Readout_Tracker] = Vars.t
                data[1][Base.Readout_Tracker] = Vars.T
                data[2][Base.Readout_Tracker] = Vars.T_global
                j = Base.Readout_Tracker
        elif (i) % Base.data_readout == 0:
            j += 1
            data[0][j] = Vars.t
    ...
    if Base.control:
        ...
        # reset
        Base.eq_condition = rk4input['eq_condition']
        Base.eq_condition_length = rk4input['eq_condition_length']
        Base.eq_condition_amplitude = rk4input['eq_condition_amplitude']
        Base.data_readout = rk4input['data_readout']
        Base.number_of_integration = rk4input['number_of_integration']
        Vars.t = 0
